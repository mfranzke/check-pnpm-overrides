...
function lintActionYmlStructure() {
  ...
  const scriptSteps = runSteps.filter(step => 
    step.run && step.run.includes('${{ github.action_path }}/scripts/')
  );
  
  for (const step of scriptSteps) {
    const scriptMatches = step.run.match(/\$\{\{ github\.action_path \}\}\/scripts\/([^\s]+\.js)/g);
    if (scriptMatches) {
      for (const match of scriptMatches) {
        const scriptName = match.replace('${{ github.action_path }}/scripts/', '');
